...
RSpec.describe VersionFooter do
  describe "info" do
    context "with stale information" do
      before do
        described_class.revision_file = Pathname.new(fixture_paths).join("REVISION").to_s
        described_class.revisions_logfile = Pathname.new(fixture_paths).join("revisions_stale.log").to_s
    ...
    context "with current information" do
      before do
        described_class.revision_file = Pathname.new(fixture_paths).join("REVISION").to_s
        described_class.revisions_logfile = Pathname.new(fixture_paths).join("revisions_current.log").to_s
        described_class.reset!
      end
  ...
  context "with rollback information" do
    before do
      described_class.revision_file = Pathname.new(fixture_paths).join("REVISION").to_s
      described_class.revisions_logfile = Pathname.new(fixture_paths).join("revisions_rollback.log").to_s
      described_class.reset!
    end
    it "detects current information" do
      info = described_class.info
